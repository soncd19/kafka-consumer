...
public final class LcKafkaConsumerBuilder<K, V> {
    ...
    /**
     * The maximum time spent in handling a single {@link ConsumerRecord} by calling
     ...
     * the created {@link LcKafkaConsumer} will use {@link } to handle records in
     ...
     * for each polled {@link ConsumerRecord} to handle the record. Please tune
    ...
    private Consumer<K, V> buildConsumer(boolean autoCommit) {
        ...
        BasicConsumerConfigs.ENABLE_AUTO_COMMIT.set(configs, Boolean.toString(autoCommit));
